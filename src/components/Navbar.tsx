...
const Navbar: React.FC = () => {
  ...
  const MobileNav = () => (
    <Sheet>
      ...
      <SheetContent
        side="right"
        className="w-[280px] sm:w-[350px] bg-card border-border"
      >
        <div className="flex flex-col gap-6 mt-8">
          {navLinks.map((link) => (
            <Link
              ...
              onClick={() =>
                trackButtonClick(`Navigation - ${link.name}`, "Mobile Menu")
              }
            >
              {link.name}
            </Link>
        ...
        {navLinks.map((link) => (
          <NavigationMenuItem key={link.href}>
            <Link
              ...
              onClick={() =>
                trackButtonClick(`Navigation - ${link.name}`, "Desktop Navbar")
              }
            >
              {link.name}
            </Link>
    ...
    >
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center">{!isMobile && <DesktopNav />}</div>
